<template>
    <div>
    <form-preview v-for="(item,index) in list" :header-label="item.studentname" :header-value="item.subject" :body-items="info[index]" :key="index"></form-preview>
    <br>
  </div>
</template>
...
export default {
  ...
  methods:{
    getList() {
      ...
      this.$http.get('http://139.199.168.158:8080/admin/teachergetcoursechangerecord?id='+this.$store.getters.userid).then((response) => {
         ...
         if (response.code === 1) {
           ...
           let temp={
             ...
             studentname:response.data[i].STUDENT,
